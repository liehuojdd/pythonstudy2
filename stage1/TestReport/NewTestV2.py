#Website:http://xlsxwriter.readthedocs.io/
#cd D:\Program Files (x86)\Python36\Scripts
#Update Pip(maybe can igroe): python -m pip install --upgrade pip
#(Not XlsWriter):pip install XlsxWriter
# -*- coding: utf-8 -*-
import xlsxwriter
...
def _write_url(worksheet, cl, data,name):
    return worksheet.write_url(cl, data,name)
...
def init(worksheet):
    ...
    data = {"test_name": "Novatus", "test_version": "v2.0.8", "test_pl": "android", "test_net": "wifi"}
    data1 = {"test_sum": 100, "test_success": 80, "test_failed": 20, "test_date": "2018-10-10 12:10"}

    # Create a new Chart object.

    worksheet.merge_range('B1:G1', 'Test Summary', define_format_H1)
    worksheet.merge_range('B2:G2', 'Test Summary', define_format_H2)
    #worksheet.merge_range('A3:A6', '这里放图片', get_format_center(workbook))

    _write_center(worksheet, "B3", 'Project', workbook)
    _write_center(worksheet, "B4", '', workbook)
    _write_center(worksheet, "B5", 'Language', workbook)
    _write_center(worksheet, "B6", 'Test Suite', workbook)
    ...
    #worksheet.write_url('C6', 'D:\P01.png', workbook)
    #worksheet.write_url('C6', 'D:\P01.png', string='Screen Short')
    #worksheet.write(row, 1, '=SUM(B2:B5)', money)

    _write_center(worksheet, "D3", "Total", workbook)
    _write_center(worksheet, "D4", "Pass", workbook)
    _write_center(worksheet, "D5", "Failed", workbook)
    _write_center(worksheet, "D6", "Test Date", workbook)
    ...
    _write_center(worksheet, "F3", 'Order Entry', workbook)
    _write_center(worksheet, "F4", 'Office', workbook)
    _write_center(worksheet, "F5",'Prepress', workbook)
    _write_center(worksheet, "F6", 'Item Mait', workbook)

    _write_center(worksheet, "G3", 'A', workbook)
    _write_center(worksheet, "G4", 'A', workbook)
    _write_center(worksheet, "G5", 'A', workbook)
    _write_center(worksheet, "G6", 'A', workbook)
...
def test_detail(worksheet):
    ...
    worksheet.merge_range('A1:M1', 'Test Detail', get_format(workbook, {'bold': True, 'font_size': 18 ,'align': 'center','valign': 'vcenter','bg_color': 'blue', 'font_color': '#ffffff'}))
    _write_center(worksheet, "A2", 'User Story', workbook)
    _write_center(worksheet, "B2", 'Test Case', workbook)
    _write_center(worksheet, "C2", 'Test Result', workbook)
    _write_center(worksheet, "D2", 'Start Time', workbook)
    _write_center(worksheet, "E2", 'End Time', workbook)
    _write_center(worksheet, "F2", 'Time Spand', workbook)
    _write_center(worksheet, "G2", 'Error Type', workbook)
    _write_center(worksheet, "H2", 'Test Detail', workbook)
    _write_center(worksheet, "I2", 'Memo', workbook)
    _write_center(worksheet, "J2", 'USTotal', workbook)
    _write_center(worksheet, "K2", 'USFailed', workbook)
    _write_center(worksheet, "L2", 'TestTotal', workbook)
    _write_center(worksheet, "M2", 'TestFailed', workbook)
    ...
    for item in data["info"]:
        ...
        #_write_center(worksheet, "J" + str(temp), item["t_result"], workbook)
